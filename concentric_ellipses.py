#Concentric ellipses classification
import numpy as np 
import torch 
...
#data generation
def generate_ellipses(n_total = 1200, n_train = 1000, seed =42):
    rng = np.random.default_rng(seed)
    n_each = n_total//2
    #internal
    th0 = rng.uniform(0,2*np.pi, n_each)
    r0 = rng.uniform(0.0, 0.6, n_each)
    X0 = np.column_stack([r0*1.0*np.cos(th0), r0*0.5*np.sin(th0)])
    #external
    th1 = rng.uniform(0, 2*np.pi, n_each)
    r1 = rng.uniform(0.61, 1.2, n_each )
    X1 = np.column_stack([r1*1.0*np.cos(th1), r1*0.5*np.sin(th1)])
    ...
    return (to_t(X[:n_train]), to_t(y[:n_train]), to_t(X[n_train:]), to_t(y[n_train:]), X[:n_train], y[:n_train])
...
if __name__ == "__main__":
   X_tr, y_tr, X_val, y_val, X_tr_np, y_tr_np = generate_ellipses(1200,1000)
   mean = X_tr.mean(0)
   std = X_tr.std(0)
   X_tr = (X_tr - mean)/std
   ...
   fig1.suptitle("Concentric Ellipses", fontsize=13, fontweight = "bold")
   ...
   fig1.savefig("concentric_ellipses_results.png", dpi=150, bbox_inches="tight")
#PLOT 2: training curves 
   fig2, axes = plt.subplots(1, 2, figsize=(12, 4))
   fig2.suptitle("Concentric Ellipses – Training Curves [Adam]",
   ...
   fig2.tight_layout()
   fig2.savefig("concentric_ellipses_curves.png", dpi=150, bbox_inches="tight")
 
    # PLOT 3: probability maps
   fig3, axes = plt.subplots(1, 2, figsize=(12, 5))
   fig3.suptitle("Concentric Ellipses – Probability Maps [Adam]",
                  fontsize=12, fontweight="bold")
   ...
   fig3.tight_layout()
   fig3.savefig("concentric_ellipses_probmaps.png", dpi=150, bbox_inches="tight")
   ...
   ax.set_title("Concentric Ellipses – Accuracy Comparison", fontsize=11)
   ax.grid(True, axis="y", alpha=0.3)
   fig4.tight_layout()
   fig4.savefig("concentric_ellipses_comparison.png", dpi=150, bbox_inches="tight")
 
   print("\nFigures saved:")
   print("  concentric_ellipses_results.png")
   print("  concentric_ellipses_curves.png")
   print("  concentric_ellipses_probmaps.png")
   print("  concentric_ellipses_comparison.png")
   plt.show() 
